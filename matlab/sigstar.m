...
%     fprintf(updown);
    ...
    for ii=1:length(groups)
        if strcmp(updown, 'up')
            thisY=findMinY(xlocs(ii,:),updown)+yd;
        else
            thisY=findMinY(xlocs(ii,:),updown)-yd;
        end
        H(ii,:)=makeSignificanceBar(xlocs(ii,:),thisY,stats(ii),updown);
    end
    %-----------------------------------------------------
...
function H=makeSignificanceBar(x,y,p,updown)
    ...
    H(2)=text(mean(x(:)),starY,stars,...
        ...
        'Tag','sigstar_stars','Color','red','FontSize',20);
